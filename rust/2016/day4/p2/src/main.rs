...
fn main() {
    ...
    //decipher real room names
    for room in rooms {
        let cipher_offset = room.sector_id % 26;
        let encrypted_name = String::from(&room.encrypted_name);
        let decyphered = caesar_cipher(encrypted_name, cipher_offset as u8);
        if decyphered.contains("northpole") {
            dbg!(&room.encrypted_name);
            dbg!(&decyphered);
            dbg!(room.sector_id);
        }
    }
